...
// The function of this class is to create a texture atlas using the graphics in
// the graphics folder, for this we want: 1 - Number of grhs 2 - Position X1 Y1
// of the grh relatively to the atlas 3 - Size of the grh 4 - Type of the grh
// (Head - Body - Weapon - Grh - Fx) The atlas max dimensions are 8000*8000, the
// idea is to create a new atlas when needed that way the client only need a few
// textures to work
...
public:
  Atlas() {
    const auto grh_container = read_graphs_initialization_file("graphics.json");
    ...
    for (const auto &[id, static_grh] : grh_container) {
      if (static_grh.graph != current_texture) {
        ...
        if (!image.loadFromFile(std::filesystem::path{"resources/graphics"} /
